...
import gc
import math
from L9_validation_error import L9_validation
...
def L8_model():
    ...
    model = Model(inputs = inputs, outputs = merged_layer8)

    return model

def L9_model(prev_model):

    # Assign the model to be the same as model L8 before adding new layers
    model = prev_model

    # Load the weights trained for model #8
    model.load_weights("L8_weights.h5")

    # Bilinear upsampling from Layer8 to 9
    conv9 = UpSampling2D(size=(2,2))(model.output)
    ...
    T_conv2 = Conv2DTranspose(filters=1, kernel_size=(1,1), strides=(1, 1), padding='valid',  activation='relu', kernel_initializer=initializers.RandomUniform(minval=-0.002, maxval=0.002, seed=None),kernel_regularizer=l2(0.0005))(model.layers[3].output)
    
    # Merging the layers
    merge_layer_9 = Add()([T_conv2,c9_conv])

    # Defining the new output of the model
    model = Model(inputs = model.input, outputs = merge_layer_9)
...
for big_epoch in range (5):
    ...
    # Instantiate a list to store the validation error for each epoch
    validation_error = []
    ...
    for img_counter in range (len(left_training)):
        ...
        for mini_epoch in range (1):
            ...
            if g_count == 0:
                ...
                h_grad_tens = tf.convert_to_tensor(h_grad.reshape((1,24,78,3)),dtype=tf.float32)
                ...
                model = L9_model(L8_model())
                model.load_weights("L9_weights.h5") 
            ...
            def loss_fn(h_grad_tens,prev_warped_image_tens,rescaled_left_image):
                def loss(predicted_disparity,prev_disparity_tens):
                    ...
                    return (K.square((((exp_predicted_disparity-exp_prev_disparity_tens)*h_grad_tens)+prev_warped_image_tens)-rescaled_left_image) + (0.01*K.square(((K.concatenate((exp_predicted_disparity[:,:,1:,:],tf.constant(np.zeros((1,24,1,3)),dtype=tf.float32)),axis=2)-exp_predicted_disparity)))))/3  
                return loss

            # Defining the optimizer
            sgd = SGD(lr=(np.true_divide(np.true_divide(1,np.power(1+np.multiply(0.0005,big_epoch+1),(big_epoch))),1400)), momentum=0.9, nesterov=False)
            ...
            # Save the model
            model.save_weights("L9_weights.h5")
            ...
            # Compute the validation error every 100 iterations
            if img_counter % 100 == 0 and img_counter != 0:
                validation_loss = L9_validation(model.get_weights(), left_validation[0:50], right_validation[0:50])
    ...
    # After each epoch has ended, save the validation error to an external file 
    error = np.asarray(validation_error)
    np.savetxt(file_path.format(big_epoch), error, delimiter=",")