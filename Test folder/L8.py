...
def L8_model(prev_model):
    # Set the model to the original coarse output
    model = prev_model

    # Load the weights to the previous model (weights computed from before)
    model.load_weights("L7_weights.h5")

    # Upsample the output
    c7_up = Cropping2D(cropping=((0,0),(0,1)))(UpSampling2D(size=(2,2))(model.output))
    ...
    T_conv5 = Conv2DTranspose(filters=1, kernel_size=(1,1), strides=(1, 1), padding='valid',  activation='relu', kernel_initializer=initializers.RandomUniform(minval=-0.002, maxval=0.002, seed=None),kernel_regularizer=l2(0.0005))(model.layers[7].output)

    # Merging the layers
    merged_layer8 = Add()([T_conv5,c7_up_conv])

    # Defining the new output of the model
    model = Model(inputs = model.input, outputs = merged_layer8)

    return model

#model = L8_model(L7_model())

#model.summary()

# Train model
train_path = "C:\\Users\\lim-z\\Desktop\\CMPT 726 Machine Learning\\Project\\Data\\Train\\Common"
...
# Remove 20% of the data for validation
...
# Disparity values that has to be carried on no matter what image currently accessing
#global prev_disparity
#global current_disparity

for big_epoch in range (2):

    for img_counter in range (len(left_training)):

        #print(left_img_pat_lst[img_counter])
        #print(right_img_pat_lst[img_counter])
        ...
        rescaled_left_image = cv2.resize(original_left_image,(39,12))
        rescaled_left_image = rescaled_left_image.reshape((1,12,39,3))
        ...
        # Down sample the left image to be the same as the right image
        rescaled_right_image = cv2.resize(original_right_image,(39,12))
        rescaled_right_image = rescaled_right_image.reshape((1,12,39,3))
        ...
        for mini_epoch in range (1):
            ...
            if g_count == 0:
                ...
                h_grad_tens = tf.convert_to_tensor(h_grad.reshape((1,12,39,3)),dtype=tf.float32)
                ...
                model = L8_model(L7_model())
                model.load_weights("L8_weights_v2.h5") 
            ...
            def loss_fn(h_grad_tens,prev_warped_image_tens,rescaled_left_image):
                def loss(predicted_disparity,prev_disparity_tens):
                    ...
                    # (0.01*K.square(((K.concatenate((predicted_disparity[:,:,1:,:],tf.constant(np.zeros((1,6,1,3)),dtype=tf.float32)),axis=2)-predicted_disparity)))) 
                    return (K.square((((exp_predicted_disparity-exp_prev_disparity_tens)*h_grad_tens)+prev_warped_image_tens)-rescaled_left_image) + (0.01*K.square(((K.concatenate((exp_predicted_disparity[:,:,1:,:],tf.constant(np.zeros((1,12,1,3)),dtype=tf.float32)),axis=2)-exp_predicted_disparity)))))/3  
                return loss

            # Defining the optimizer
            sgd = SGD(lr=(np.true_divide(np.true_divide(1,np.power(1+np.multiply(0.0005,big_epoch+1),(big_epoch))),400)), momentum=0.9, nesterov=False)
            print(np.true_divide(np.true_divide(1,np.power(1+np.multiply(0.0005,big_epoch+1),(big_epoch))),400))
            # Compile the model
            ...
            #model.fit(x=input_left_image, y=prev_disparity,epochs=1)

            # Save the model
            model.save_weights("L8_weights_v2.h5")
            ...
            # Compute the validation error every 10 iterations
            #if g_count % 10 == 0 and g_count != 0:
                #validation_loss = model.evaluate(x=input_left_image, y=prev_disparity, verbose=0)
